...
COMPOSE_FILE="game-server-wireguard-relay-docker-compose.yml"
...
if [ "$new_major" != "$cur_major" ] && [ "$ALLOW_MAJOR" != "true" ]; then
  ...
  echo "  https://github.com/heyvaldemar/game-server-wireguard-relay-docker-compose/releases/tag/$latest" >&2
  exit 3
fi
